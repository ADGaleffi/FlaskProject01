...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Email is not connected with an account', category='error')
...
@auth.route('/lectures', methods=['GET', 'POST'])
def lectures():
    return render_template("lectures.html", user=current_user)

@auth.route('/article1', methods=['GET','POST'])
def article1():
    return render_template("article1.html", user=current_user)

@auth.route('/article2', methods=['GET','POST'])
def article2():
